...
def authoring_evidence() -> dict:
    return {
        ...
        "sharedHarness": source_metrics(
            [HERE / "protocol.py", HERE / "run_suite.py", HERE / "async_utils.py"]
        ),
        "native": source_metrics([HERE / "native_executor.mjs"]),
        "rocketride": source_metrics(
            [
                HERE / "rocketride-node" / "nodeworkflow" / "IGlobal.py",
                HERE / "rocketride-node" / "nodeworkflow" / "IInstance.py",
                HERE / "rocketride-node" / "nodeworkflow" / "__init__.py",
                HERE / "rocketride-node" / "nodeworkflow" / "services.json",
            ]
        ),
        "langchain": source_metrics([HERE / "langchain_executor.py"]),
        "verification": source_metrics(
            [HERE / "audit_run.py", HERE / "run_app_verifiers.py", HERE / "test_protocol.py"]
        ),
    }
    ...
    for fixture in fixture_records:
        ...
        for variant in ("normal", "hard-failure"):
            for framework in frameworks:
                ...
                deadline_overruns = deadline_overrun_count(results, deadline_ms)
                ...
                if deadline_overruns:
                    gate_issues.append(
                        f"{fixture['fixtureId']} {variant} {framework}: "
                        f"{deadline_overruns}/{len(results)} repetitions exceeded the "
...
def report(scorecard: dict) -> str:
    lines = [
        ...
        "",
        "| App | Variant | Framework | Completed units median | Failed units median | Deadline overruns | Total p50 ms | Total p95 ms | Warm-up p50 ms |",
        "|---|---|---|---:|---:|---:|---:|---:|---:|",
    ]
    ...
    for surface in ("sharedHarness", "native", "rocketride", "langchain", "verification"):
    ...
    lines.extend(
        [
            ...
            "- Every result must complete within the fixture deadline; an overrun fails the runner gate.",
...
async def run(args: argparse.Namespace) -> int:
    ...
    write_json(
        ...
        {
            "runId": run_id,
            "path": run_root.relative_to(REPO).as_posix(),
